...
import (
	"context"
	"github.com/ingkagero/rest-layer-datastore"
...
func main() {
	...
	client, err := datastore.NewClient(ctx, os.Getenv("APP_ENGINE_PROJECT"))
	if err != nil {
		log.Fatalf("Error connecting to Datastore: %s", err)
	}
	// create datastore handler
	storeHandler := datastore.NewHandler(client, "default", "users")
	// Add a resource on /users[/:user_id]
	users := index.Bind("users", user, storeHandler, resource.Conf{
